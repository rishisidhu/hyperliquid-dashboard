...
### Deployed — 2026-07-xx (live)
- **Live URLs:** frontend **https://niminal.xyz** (Vercel) · backend **https://api.niminal.xyz** (DigitalOcean droplet).
- **Split architecture (as built):** browser → **static Next.js export on Vercel** (global CDN, no server runtime) → streams from a **single Node poller + SSE fan-out on the DO droplet** (`127.0.0.1:8765`, behind nginx as `api.niminal.xyz`), which polls the Hyperliquid public API on one fixed schedule. **Upstream load is constant regardless of viewer count** — every browser talks to us, one backend talks to Hyperliquid. The backend co-exists with the aigraduate.com blog on the same 1 GB/1 vCPU box, hard-capped by systemd (`MemoryMax=160M`/`CPUQuota=35%`/`TasksMax=64`) so it can never starve the blog. OI history persists in a local SQLite file under the service user's own dir.
- **Deploy stack:** systemd unit (unprivileged `hldash` user, sandboxed) + new isolated nginx server block + acme.sh TLS (separate webroot, LE) — all additive, never touching the blog's nginx/cert/webroot. Runbook: `backend/deploy/RUNBOOK.md`.
- **Outstanding:** 🔴 acme.sh renewal self-truncation MUST-FIX before ~Sept 2026 (see RUNBOOK §4) — the only known follow-up.
